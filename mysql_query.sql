...
Перелік_зупинок INTEGER (8),
		     FOREIGN KEY (Номер_потягу) REFERENCES Потяги (Номер_потягу) ON DELETE CASCADE,
		     FOREIGN KEY (Номер_маршруту) REFERENCES Маршрути (Номер_маршруту) ON DELETE CASCADE,
                     PRIMARY KEY (id));
#4.Вагони:
CREATE TABLE Вагони (
id MEDIUMINT UNSIGNED NOT NULL AUTO_INCREMENT,
...
FOREIGN KEY (Номер_потягу) REFERENCES Потяги (Номер_потягу) ON DELETE CASCADE,
FOREIGN KEY (Номер_вагону) REFERENCES Вагони (id) ON DELETE CASCADE,
PRIMARY KEY (id));
#6.Пасажири:
...
FOREIGN KEY (Номер_маршруту) REFERENCES Маршрути (Номер_маршруту) ON DELETE CASCADE,
FOREIGN KEY (Номер_потягу) REFERENCES Потяги (Номер_потягу) ON DELETE CASCADE,
FOREIGN KEY (Номер_дня_відправлення) REFERENCES Дні_відправлення (id) ON DELETE CASCADE,
FOREIGN KEY (Номер_місця) REFERENCES Місця (id) ON DELETE CASCADE,
FOREIGN KEY (Номер_пасажира) REFERENCES Пасажири (id) ON DELETE CASCADE,
FOREIGN KEY (Номер_вагону) REFERENCES Вагони (id) ON DELETE CASCADE,
